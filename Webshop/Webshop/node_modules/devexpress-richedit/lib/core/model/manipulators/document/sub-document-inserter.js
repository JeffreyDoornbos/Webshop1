...
var SubDocumentInserterOptions = (function (_super) {
    ...
    function SubDocumentInserterOptions() {
        ...
        _this.overlapTableCellContent = false;
        _this.numberingListCache = {};
        _this.abstractNumberingListCache = {};
        return _this;
    }
...
var SubDocumentInserter = (function () {
    ...
    SubDocumentInserter.prototype.insert = function () {
        ...
        this.history.addTransaction(function () {
            ...
            if (_this.options.overlapTableCellContent) {
                var lastInsertedParagraph = _this.targetSubDocument.getParagraphByPosition(_this.currInsertSubDocumentPosition.position - 1);
                var nextParagraph = _this.targetSubDocument.getParagraphByPosition(_this.currInsertSubDocumentPosition.position);
                nextParagraph.copyFrom(lastInsertedParagraph);
                var lastInsertedParagraphMarkInterval = new sub_document_1.SubDocumentInterval(_this.targetSubDocument, new fixed_1.FixedInterval(_this.currInsertSubDocumentPosition.position - 1, 1));
                _this.targetModelManipulator.range.removeInterval(lastInsertedParagraphMarkInterval, false, false, false);
            }
    ...
    SubDocumentInserter.prototype.collectTables = function () {
        var sourceInterval = this.sourceSubDocInterval.interval;
        sourceInterval = new fixed_1.FixedInterval(sourceInterval.start, sourceInterval.length - (this.options.overlapTableCellContent ? 1 : 0));
        var sourceTables = this.sourceSubDocument.tables;
        var startTblInd = Math.max(0, search_1.SearchUtils.normedInterpolationIndexOf(sourceTables, function (t) { return t.getStartPosition(); }, sourceInterval.start));
        ...
        for (var tblInd = startTblInd, sourceTbl = void 0; sourceTbl = sourceTables[tblInd]; tblInd++) {
            ...
            if (sourceTableInterval.start >= sourceInterval.end)
                break;
            var intersection = algorithms_1.IntervalAlgorithms.getIntersectionNonNullLength(sourceTbl.interval, sourceInterval);
    ...
    SubDocumentInserter.prototype.getParagraphListInfo = function (sourceParagraph) {
        ...
        var numberingListIndex = sourceParagraph.getNumberingListIndex();
        if (numberingListIndex >= 0) {
            if (this.sameModel) {
                var sourceNumbList = this.sourceDocumentModel.numberingLists[numberingListIndex];
                ...
                var targetIndex = this.getListIndex(this.numberingListCache, this.targetDocumentModel.numberingLists, numberingListIndex);
                if (targetIndex !== undefined)
                    ...
                    result.numberingListIndex = this.targetModelManipulator.numberingList.addNumberingList(targetNumberingList);
                    var targetNumbListId = this.targetDocumentModel.numberingLists[result.numberingListIndex].getId();
    ...
    SubDocumentInserter.prototype.getAbstractNumberingListId = function (sourceAbstrNumbListIndex) {
        var targetIndex = this.getListIndex(this.abstractNumberingListIndexesMap, this.targetDocumentModel.abstractNumberingLists, sourceAbstrNumbListIndex);
        if (targetIndex !== undefined)
            return targetIndex;
        var targetAbstrNumbList = new numbering_list_1.AbstractNumberingList(this.targetDocumentModel);
        ...
        var targetNumberingListId = this.targetDocumentModel.abstractNumberingLists[targetIndex].getId();
        this.abstractNumberingListIndexesMap[targetNumberingListId] = sourceAbstrNumbListIndex;
        return targetIndex;
    };
    SubDocumentInserter.prototype.getListIndex = function (cache, list, sourceIndex) {
        var targetIndex = number_1.NumberMapUtils.keyBy(cache, function (val) { return val == sourceIndex; });
        return common_1.isDefined(targetIndex) ? list_1.ListUtils.indexBy(list, function (list) { return list.getId() == targetIndex; }) : undefined;
    };
