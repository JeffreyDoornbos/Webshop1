...
var CreateRangeCopyOperation = (function () {
    function CreateRangeCopyOperation(subDocument, abstractNumberingListCache, numberingListCache) {
    ...
    Object.defineProperty(CreateRangeCopyOperation.prototype, "documentModel", {
        get: function () {
    ...
    CreateRangeCopyOperation.prototype.copyMainContent = function (interval, newDocumentModel, currIntervalOffset) {
        ...
        var _loop_1 = function () {
            oldCurrentRun = constRunIterator.currentRun;
            if (this_1.oldCurrentSection != constRunIterator.currentSection) {
                this_1.oldCurrentSection = constRunIterator.currentSection;
                this_1.newCurrentSection = this_1.appendNewSection(newSubDocument);
            }
            if (this_1.oldCurrentParagraph != oldCurrentRun.paragraph) {
                this_1.oldCurrentParagraph = oldCurrentRun.paragraph;
                this_1.newCurrentParagraph = this_1.appendNewParagraph(newSubDocument);
            }
            if (this_1.oldCurrentChunk != constRunIterator.currentChunk) {
                this_1.oldCurrentChunk = constRunIterator.currentChunk;
                this_1.newCurrentChunk = this_1.appendNewChunk(newSubDocument);
                this_1.newOffsetAtStartChunk = 0;
            }
            this_1.newCurrentParagraph.length += oldCurrentRun.getLength();
            this_1.newCurrentSection.setLength(newSubDocument, this_1.newCurrentSection.getLength() + oldCurrentRun.getLength());
            this_1.newCurrentChunk.textBuffer += this_1.oldCurrentChunk.getTextInChunk(oldCurrentRun.startOffset, oldCurrentRun.getLength());
            if (oldCurrentRun.getType() == run_type_1.RunType.FieldCodeStartRun)
                this_1.appendField(newSubDocument, oldCurrentRun, interval.start - currIntervalOffset);
            else if (oldCurrentRun.getType() == run_type_1.RunType.AnchoredTextBoxRun) {
                var newTexBoxSubDoc_1 = this_1.documentModel.subDocuments[oldCurrentRun.subDocId].clone(newDocumentModel);
                var sourceTextBoxSubDocParagraphs_1 = this_1.documentModel.subDocuments[oldCurrentRun.subDocId].paragraphs;
                list_1.ListUtils.forEach(newTexBoxSubDoc_1.paragraphs, function (paragraph, index) {
                    paragraph.numberingListIndex = CreateRangeCopyOperation.getNewNumberingListIndex(_this.documentModel, sourceTextBoxSubDocParagraphs_1[index].numberingListIndex, newTexBoxSubDoc_1, _this.abstractNumberingListCache, _this.numberingListCache);
                });
            }
            this_1.newCurrentChunk.textRuns.push(oldCurrentRun.createSimularity(this_1.newOffsetAtStartChunk, oldCurrentRun.getLength(), this_1.newCurrentParagraph, newDocumentModel.stylesManager.addCharacterStyle(oldCurrentRun.characterStyle), newDocumentModel.cache.maskedCharacterPropertiesCache.getItem(oldCurrentRun.maskedCharacterProperties)));
            this_1.newOffsetAtStartChunk += oldCurrentRun.getLength();
        };
        var this_1 = this, oldCurrentRun;
    ...
    CreateRangeCopyOperation.prototype.copyToCore = function (targetStyle, oldStyle, newSubDocument) {
        ...
        else if (oldStyle.numberingListIndex == numbering_list_1.AbstractNumberingList.NoNumberingListIndex) {
            targetStyle.numberingListIndex = numbering_list_1.AbstractNumberingList.NoNumberingListIndex;
        }
    };
    CreateRangeCopyOperation.prototype.copyPropertiesTo = function (targetStyle, oldStyle, newSubDocument) {
    ...
    CreateRangeCopyOperation.prototype.appendNewParagraph = function (newSubDocument) {
    ...
    CreateRangeCopyOperation.getNewNumberingListIndex = function (oldmodel, oldNumberingListIndex, newSubDocument, abstractNumberingListCache, numberingListCache) {
        if (abstractNumberingListCache === void 0) { abstractNumberingListCache = {}; }
        if (numberingListCache === void 0) { numberingListCache = {}; }
        var newCurrentParagraphNumberingListIndex = -1;
        var newAbstractNumberingListIndex = -1;
        if (oldNumberingListIndex >= 0) {
            var oldNumberingList = oldmodel.numberingLists[oldNumberingListIndex];
            ...
            if (newCurrentParagraphNumberingListIndex === undefined) {
                var oldAbstractNumberingList = oldmodel.abstractNumberingLists[oldNumberingList.abstractNumberingListIndex];
...
var RangeCopy = (function () {
    function RangeCopy(model, addedUselessParagraphMarkInEnd, abstractNumberingListCache, numberingListCache) {
        if (abstractNumberingListCache === void 0) { abstractNumberingListCache = {}; }
        if (numberingListCache === void 0) { numberingListCache = {}; }
        this.model = model;
    ...
    RangeCopy.prototype.insertTo = function (modelManipulator, subDocPosition, overlapTableCellContent) {
        if (overlapTableCellContent === void 0) { overlapTableCellContent = false; }
        ...
        var insertOptions = new sub_document_inserter_1.SubDocumentInserterOptions();
        ...
        return modelManipulator.subDocument.insertSubDocument(subDocPosition, new sub_document_1.SubDocumentInterval(this.model.mainSubDocument, sourceInterval), insertOptions)
            .insetedInterval;
    };
    ...
    RangeCopy.prototype.overlapTableCellContent = function (modelManipulator, sourceTable, targetTableInfo, rowIndex, columnIndex) {
        ...
        var copyOperation = new CreateRangeCopyOperation(this.model.mainSubDocument, this.abstractNumberingListCache, this.numberingListCache);
        ...
        rangeCopy.insertTo(modelManipulator, new sub_document_1.SubDocumentPosition(modelManipulator.model.mainSubDocument, targetCell.interval.start), true);
    };
