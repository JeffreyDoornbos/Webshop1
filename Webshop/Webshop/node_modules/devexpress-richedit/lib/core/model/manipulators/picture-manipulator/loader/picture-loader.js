...
var PicSizeUpdater = (function () {
    ...
    PicSizeUpdater.prototype.addSizes = function (subDocPos, options, run, histItemSize) {
        ...
        var pos = subDocPos.subDocument.positionManager.registerNotLoadedPicturePosition(subDocPos.position);
...
var PictureLoader = (function () {
    ...
    PictureLoader.prototype.loadPictureByBase64 = function (data, imageLoaded) {
        ...
        img.onload = function () {
            ...
            var contentType = extension_helper_1.ExtensionHelper.getMimeTypeFromBase64Uri(data.base64);
            if (contentType == 'image/bmp')
                _this.convertBmpToPng(data, img);
            imageLoaded(data);
        };
        img.src = data.base64;
    };
    PictureLoader.prototype.convertBmpToPng = function (data, img) {
        var canvas = document.createElement('canvas');
        if (!canvas)
            return;
        var originalSize = data.size.clone().applyConverter(unit_converter_1.UnitConverter.twipsToPixelsF);
