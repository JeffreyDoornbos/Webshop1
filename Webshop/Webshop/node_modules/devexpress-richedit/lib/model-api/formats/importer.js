...
function getAfterInsertCallback(processor, subDocument, position, callback) {
    return function (model, formatImagesImporter) {
        formatImagesImporter.whenAllPicturesLoaded(function (_success) {
            ...
            var result = processor.modelManager.modelManipulator.subDocument.insertSubDocument(new sub_document_1.SubDocumentPosition(subDocument, position), new sub_document_1.SubDocumentInterval(model.mainSubDocument, model.mainSubDocument.interval));
            model.updateHyperlinkFields(processor, new sub_document_1.SubDocumentInterval(subDocument, result.insetedInterval), result.newSubDocuments);
            processor.endUpdate();
