...
var switch_text_box_sub_documents_state_history_item_1 = require("../../../core/model/history/items/switch-text-box-sub-documents-state-history-item");
...
var FloatingObjectDragDropChangePositionCommand = (function (_super) {
    ...
    FloatingObjectDragDropChangePositionCommand.prototype.removeOldRun = function (oldRun, oldRunPos) {
        var subDoc = oldRun.paragraph.subDocument;
        this.rangeCopy = create_range_copy_operation_1.RangeCopy.create(new sub_document_1.SubDocumentIntervals(subDoc, [fixed_1.FixedInterval.fromPositions(oldRunPos, oldRunPos + 1)]));
        var interval = new sub_document_1.SubDocumentInterval(subDoc, new fixed_1.FixedInterval(oldRunPos, 1));
    ...
    FloatingObjectDragDropChangePositionCommand.prototype.addRun = function (oldRun, position, anchorInfo) {
        if (this.rangeCopy) {
            var intermediateRun = this.rangeCopy.model.mainSubDocument.getFirstRun();
            intermediateRun.anchorInfo = anchorInfo.clone();
            this.rangeCopy.insertTo(this.modelManipulator, new sub_document_1.SubDocumentPosition(this.activeSubDocument, position));
        }
        this.rangeCopy = null;
        var newRun = this.activeSubDocument.getRunAndIndexesByPosition(position).run;
        if (newRun.getType() == run_type_1.RunType.AnchoredTextBoxRun) {
            var oldTextBoxRun = oldRun;
            var originalSubDocument = this.control.modelManager.model.subDocuments[oldTextBoxRun.subDocId];
            this.history.addAndRedo(new switch_text_box_sub_documents_state_history_item_1.SwitchTextBoxSubDocumentsStateHistoryItem(this.modelManipulator, originalSubDocument, this.activeSubDocument, position));
        }
    };
