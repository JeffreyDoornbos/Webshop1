        ...
        if (newSubDocument.isTextBox() && (!selection.specialRunInfo.isSelectedAnchorObject ||
            ...
            do
                if (iterator.run.getType() == run_type_1.RunType.AnchoredTextBoxRun && iterator.run.subDocId == newSubDocument.id)
                    break;
            while (iterator.moveToNextRun());
            selection.specialRunInfo.init(selection_intervals_info_1.SelectionIntervalsInfo.fromInterval(parentSubDoc, new fixed_1.FixedInterval(iterator.getAbsolutePosition(), 1)), this.modelManipulator.model);
        }
        return true;
    };
