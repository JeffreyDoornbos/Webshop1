            ...
            new rtf_importer_1.RtfImporter(options).importFromString(rtf, _this.control.modelManager.richOptions, function (model, formatImagesImporter) {
                ...
                formatImagesImporter.whenAllPicturesLoaded(function (_success) {
                    ...
                    _this.control.modelManager.history.addTransaction(function () {
                        command_base_1.CommandBase.addSelectionBefore(_this.control);
                        _this.control.modelManager.modelManipulator.range.removeInterval(new sub_document_1.SubDocumentInterval(subDocument, interval), true, true);
                        var result = _this.control.modelManager.modelManipulator.subDocument.insertSubDocument(new sub_document_1.SubDocumentPosition(subDocument, interval.start), new sub_document_1.SubDocumentInterval(model.mainSubDocument, model.mainSubDocument.interval));
                        command_base_1.CommandBase.addSelectionAfter(_this.control, result.insetedInterval.end);
                        model.updateHyperlinkFields(_this.control, new sub_document_1.SubDocumentInterval(subDocument, result.insetedInterval), result.newSubDocuments);
                    });
