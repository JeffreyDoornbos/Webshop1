...
var HtmlExporter = (function () {
    ...
    HtmlExporter.prototype.getHtmlElementsByInterval = function (model, subDocument, interval, guidLabel) {
        ...
        while (iterator.moveNext()) {
            ...
            switch (run.getType()) {
                ...
                case run_type_1.RunType.AnchoredTextBoxRun: {
                    var textBoxRun = run;
                    var internalSubDocument = model.subDocuments[textBoxRun.subDocId];
                    innerHtml = "<table " + guidLabel + " border='1' style='border-width: 0px; border-collapse: collapse; border-spacing: 0px;'><tbody><tr>";
                    innerHtml += "<td style='" + this.getTextBoxStyleString(textBoxRun) + "'>";
                    innerHtml += this.getHtmlElementsByInterval(model, internalSubDocument, new fixed_1.FixedInterval(0, internalSubDocument.getDocumentEndPosition()), guidLabel);
            ...
            if (hyperlinkInfo && !isInsideHyperlink) {
                ...
                html = "<a href=\"" + url + "\" title=\"" + tooltip + "\">" + hyperlinkInnerHtml + "</a>";
                hyperlinkInfo = null;
                hyperlinkInnerHtml = "";
            }
            if (!html && innerHtml) {
